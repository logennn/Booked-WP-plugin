...
<div class="condition-block customer_choice<?php echo ( $guest_booking || get_option('users_can_register') && !is_user_logged_in() || is_user_logged_in() ? ' default' : '' ); ?>" id="condition-new">
	<form action="" method="post" id="newAppointmentForm">
		...
		<div class="field">
			...
			<?php else: ?>
				<input type="submit" id="submit-request-appointment" class="button button-primary" value="<?php echo ( $new_appointment_default == 'draft' ? esc_html( _n( 'Request Appointment', 'Request Appointments', $total_appts, 'booked' ) ) : esc_html( _n( 'Lefoglalom', 'Book Appointments', $total_appts, 'booked' ) ) ); ?>">
				<button class="cancel button"><?php esc_html_e('Cancel','booked'); ?></button>
			<?php endif; ?>
		</div>
	</form>
